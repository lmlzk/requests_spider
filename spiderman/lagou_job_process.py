...
class LaGou(object):
    ...
    def __init__(self, search, city='北京', exp=WORK_EXP['3年及以下']):
        self.search = search
        self.city = city
        self.exp = exp
    ...
    def crawl_desc(self):
        ...
        print("salary: "+cur_data['salary'])
        crawl_desc = JobDesc(url)
        detail = crawl_desc.run()
        data = dict(detail, **cur_data)
    ...
    def parse_data(self):
        ...
        for node in node_list:
            ...
            temp['job'] = work_node.find_element_by_xpath('./h3').text
            temp['com_place'] = work_node.find_element_by_xpath('./span').text
            temp['job_url'] = work_node.get_attribute('href')
    ...
    def save_data(self):
        while True:
            ...
            print("save_file")
            with open('lagou_'+self.city+'_'+self.search+'.json', 'a')as f:
                res = json.dumps(data, ensure_ascii=False) + ',\n'
                f.write(res)
            self.fin_queue.task_done()

    def run(self):
        self.go_to_url(self.city, self.exp, WORK['全职'])
        page_len = self.drive.find_element_by_xpath('//span[@page][last()]').text
        print("page_total_len: "+str(page_len))
